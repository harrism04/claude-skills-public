...
This reference has ready-to-paste handler snippets for the integrations 8x8 SEs use most often.
...
The lightest-weight option. Used when the demo wants to show the conversation transferring from automated WhatsApp into a live agent's queue, and the SE has a Moobidesk tenant they can log into during the meeting. The audience sees the demo hand off and then sees the real tool open in another tab — the live system is the payoff.
...
**Gotcha**: customer-specific Moobidesk tenants often sit behind SSO. If the SE isn't logged in during the dry run, the click-out 404s. Always log in before the meeting.
...
This pattern is more work (you're rebuilding the inbox UI) but vastly higher-fidelity. The audience gets to watch replies land, tap auto-categorised conversations, fire smart-reply chips, and feel the full ops workflow without the SE juggling tabs.
...
| SE has real Moobidesk tenant to demo? | Yes, use it | No, or audience can't see it |
...
After the audience completes the MockPass flow, they tab back to the demo and the SE clicks "Simulate verification complete" (a `customer_reply` step) to advance.
...
If the SE wants to fire a *real* OTP during the demo (instead of a simulated one), they can call the 8x8 CPaaS MCP `initiate_verification` tool out-of-band before the meeting — the demo just shows what landed.

---

## 8x8 ChatApps API (live WhatsApp send during demo)

Some SEs want to fire a real WhatsApp template during the demo (instead of just the simulated chat). Pattern:

1. Use the `8x8-cpaas-se-toolkit:business-messaging` skill to send the template to the SE's own phone before/during the meeting.
2. Project the SE's phone screen alongside the demo HTML so the audience sees the message land for real.
...
Don't try to fire live sends from the HTML itself. Browser CORS will block direct API calls, and embedding API keys in client-side HTML is a security gun pointed at the SE's foot.
...
- **End with a "complete" button** that closes the tab or shows a "Return to WhatsApp" message. The actual round-trip is the SE clicking "Simulate verification complete" back in the orchestrator.
...
- **Companion page doesn't open** — popup blocker. Tell the SE to allow popups for the demo URL during their dry-run.
...
- **Moobidesk URL behind SSO** (Pattern A) — the trial URL is open, but customer-specific Moobidesk tenants may require SSO. If the SE isn't logged in, the click-out 404s. Ask them to log in to Moobidesk in the same browser before the demo.
